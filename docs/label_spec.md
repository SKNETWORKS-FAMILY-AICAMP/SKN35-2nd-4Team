...
| `moderate_performance_decline` | 위 태그에 속하지 않고 전력 변화가 관측되며 강한 하락 임계와 0 사이 |
| `limited_history` | 위 태그에 속하지 않고 직전 비교 시즌이 없어 변화량 계산 불가 |
...
5. 전력 변화가 음수지만 강한 하락 임계에는 못 미치면
   `moderate_performance_decline`을 적용한다.
6. 직전 비교 시즌이 없어 변화량 자체를 계산할 수 없으면 `limited_history`로
   표시한다. 이는 이탈 원인이 아니라 데이터 가용 상태이므로 모델 학습에서는
   제외하고 서비스에서 규칙으로 표시한다.
...
| `moderate_performance_decline` | 1,053 | 9.21% |
| `career_stage` | 810 | 7.08% |
| `injury_associated` | 488 | 4.27% |
| `limited_history` | 326 | 2.85% |

기존 `unknown` 5,500건(48.09%) 중 저연차 2,215건과 전력 유지·상승이 관측된
1,906건을 분리했다. 최신 부상 위험점수 재계산 후 남은 1,379건도 완만한 전력
하락 1,053건과 비교 이력 부족 326건으로 분리하여 `unknown`은 0건이다.

### 7.3 관측된 이탈 사건 — 원인 예측과 분리

트레이드·FA·방출은 사건 이후에 알게 되는 사후 결과다. 이를 원인 모델의 정답으로
직접 사용한 실험에서 RF test Macro F1이 0.86 수준에서 0.59로 하락했고,
트레이드 F1 0.14·마이너 옵션 F1 0.00이 확인됐다. 따라서 미래 거래유형을 현재
성적만으로 예측하지 않고 아래 값을 `departure_event_type`으로 별도 보존한다.

| 사건 값 | 2026-08-30 건수 | 근거 |
|---|---:|---|
| `roster_release_waiver` | 6,150 | 방출·DFA·웨이버 또는 L2b 방출 |
| `free_agent_market` | 2,475 | FA 선언·계약 또는 L2b FA |
| `transaction_trade` | 906 | L2 트레이드와 실제 거래 기록 |
| `injury_roster_move` | 865 | 시즌 말 IL·미복귀 로스터 이동 |
| `league_exit` | 776 | 다른 사건이 확인되지 않은 리그 이탈 |
| `minor_league_option` | 264 | 마이너 옵션 기록 |

트레이드는 해당 시즌 달력연도, 오프시즌 사건은 9~12월을 해당 시즌에, 1~4월을
직전 시즌에 귀속한다. `departure_event_type`과 거래 확인 수준은 모델 피처에서
제외하여 미래정보 누수를 막는다.
...
`limited_history`는 모델 예측이 아니라 현재 행의 직전 시즌 변화량 결측을 직접
확인해 표시한다. 그 외에는 최고 원인 확률이 서비스 임계값보다 낮을 때만
"판단 근거 부족"을 표시한다. 미래 라벨이 없다는 이유만으로 이 문구를 표시하지
않는다.
...
- `limited_history`는 라벨 분포에는 포함하되 모델 성능 산출에서는 제외한다.
